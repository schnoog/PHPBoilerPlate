...
if (isset($_POST['action'])){
    $action = $_POST['action'];
    if (strpos($action,"_")) list($action,$actid) = explode("_",$action);
}
...
//editform
            case "editform" :
                    ...
                    for ($x=0;$x<count($dump);$x++){ $navitems[$dump[$x]['id']] = $dump[$x]['nav_title'];  }
                    $dump = DB::query("Select * from nav_types");
                    for ($x=0;$x<count($dump);$x++){ $navtypes[$dump[$x]['id']] = $dump[$x]['navtype'];  }
...
//navsave
            case "navsave" :
                    ...
                    for ($x=0;$x < count($savedata);$x++){
                        ...
                        $setparent = 0;
                        if (strlen($item['parent_id'])>0)$setparent = intval($item['parent_id']);
...
//savenavitem                    
            case "savenavitem" :
                        $nooutput = true;
                            ...
                            $save['nav_sort'] = 5;
                            if (isset($_REQUEST['nav_sort']))$save['nav_sort'] = intval($_REQUEST['nav_sort']);
...
////////////////////////////////////////////////////////////// +++Tools
            case "createpage" :
                    ...
                    $withjs = true;
                    if (strlen($pagename)<3) $pagename= '!")(&%"';
                    if ($_REQUEST['withjs'] == "No") $withjs = false;
