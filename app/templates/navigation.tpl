...
{$bt = 'info'}
{/if}        
			<td><a href="{$navdata.login.socialpage}?provider={$provider}" class="btn btn-sm btn-block btn-{$bt}">
                    <i class="fas fa-sign-in-alt"></i> 
                    {$provider}
            </a></td>
            {if $x is even}</tr><tr>{/if}
{/foreach}
</tr></table>
        ...
        or{/if}{/if}
            								 <form class="form" role="form" method="post" action="{$navdata.login.logintarget}" accept-charset="UTF-8" id="login-nav">
            										<div class="form-group">
            											 ...
            											 <input name="email" type="email" autocomplete="email" class="form-control" id="exampleInputEmail2" placeholder="{'Email address'|gettext}" required>
            										</div>
            										<div class="form-group">
            											 <label class="sr-only" for="exampleInputPassword2">{'Password'|gettext}</label>
            											 <input name="password" type="password" autocomplete="current-password" class="form-control" id="exampleInputPassword2" placeholder="{'Password'|gettext}" required>
