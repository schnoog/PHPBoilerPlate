...
function fGetRoles()
{
    ...
    $ret=array();
    foreach ($tmp as $rolename => $value) {
        if (in_array($value, $Settings['roles']['available'])) {
            $ret[$value] = $rolename;
        }
    }
    return $ret;
...
{
    global $Settings;
    if ($roleflag & $Settings['admin']['rootrole']) {
        return true;
    }
    return false;
}
