    ...
    for ($x=0;$x<count($rows);$x++){	
     $row = $rows[$x];
        ...
        if($debug){
            ...
            $display = false;
            if (($row['nav_allowedmask'] & $userrole) && ($row['nav_active'] == 1)) $display = true;
                
            if (($row['nav_allowedmask'] == 0) && ($row['nav_active'] == 1)) $display = true;
        }
            
        $tg ="";    
        if ($row['nav_navtype'] == 2) $tg = " target='_blank'";
	    if($display){  
