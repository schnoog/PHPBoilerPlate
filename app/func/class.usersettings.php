...
class Usersettings{
    ...
    public function parseUserIn(){
        if (count($this->presets)<1) $this->loadPresets();
        unset($this->settings);
    ...
    public function saveUserSettingIfValid($SettingKey,$SettingValue){
        ...
        if ((strlen($preset['usersetting_preset'])>0) && $preset['usersetting_limittopreset'] == 1 ){
            ...
            foreach ($availsets as $key => $value){
                if (!is_numeric($key)){
                    if ($key == $SettingValue) $isin = true;
                }
                if ($value == $SettingValue) $isin = true;
            }
            if (!$isin){
    ...
    public function getSettings($limitid=0){
        if(!$this->bolIsParsedIn) $this->parseUserIn();
        if ($limitid == 0) return $this->settings;
